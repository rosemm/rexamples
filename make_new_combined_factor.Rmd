...
For example, let's say you collected data from 20 participants asking them about whether or not they've experienced discrimination in a few different contexts: at work, at school, and in the community in general. 
...
data <- data.frame(dis_work=sample(c(0,1), size=20, replace=T), 
                   dis_school=sample(c(0,1), size=20, replace=T),
                   dis_comm=sample(c(0,1), size=20, replace=T))
...
View(data) # take a look at the new variable
# do the other two 
...
str(data)
data$trauma_type <- as.factor(data$trauma_type) # make it a factor
levels(data$trauma_type)
```

Hooray! 
Of course, the levels for this factor are kinda ugly. 
You can fix that by renaming the fatcor levels with `factor()`.